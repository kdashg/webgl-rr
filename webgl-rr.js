    ...
    */
    ...
    function Pickle(anyVal) {
        ...
        if (kMediaElemCtors.indexOf(ctor) != -1) {
            const newSnapshot = CMediaSnapshot.From(anyVal);
            const hash = MurmurHash3_x86_32(newSnapshot.dataURL);

            if (mediaSnapshots[hash] === undefined) {
                mediaSnapshots[hash] = newSnapshot;
            }
            return TagForRemap(mediaSnapshots[hash]);
        }
    ...
    function Export() {
        ...
        for (var k in mediaSnapshots) {
            ...
            const tag = TagForRemap(snapshot);
            parts.push('\n    "' + tag.id, '": "', snapshot.dataURL, '"');
        }
